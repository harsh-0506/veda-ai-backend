...
app.post("/api/generate", upload.single("document"), async (req, res) => {
  ...
  try {
      ...
      try {
        if (
          ...
          if (extractedText.length > MAX_CHARS) {
            console.log(
              `⚠️ Document too large! Truncating from ${extractedText.length} down to ${MAX_CHARS} characters.`,
            );
          ...
          hasDocument = true;
          console.log(
            `✅ PDF Extracted and ready! Sending ${extractedText.length} characters to AI.`,
          );
        } else {
    ...
    if (hasDocument && extractedText.trim().length < 300) {
      console.log(
        "⚠️ WARNING: Extremely low character count detected. Flagging document as unreadable for AI.",
      );
      isTextGarbage = true;
    }
    ...
    const prompt = `
        ...
        4. Match the Following: 
            - If a section is "Match the following", do NOT generate individual questions. 
            - Generate ONE single question object for the entire section.
            - The "matchPairs" array MUST contain exactly 10 pairs (Column A to Column B mappings) to satisfy the teacher's request for 10 rows.
            - Do not generate 10 separate questions; generate 1 complex question that holds 10 pairs.
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a JSON-generating assistant. You only output valid JSON.",
        },
    ...
    res
      .status(500)
      .json({ status: "error", message: "Failed to generate assignment." });
  }
});

app.listen(5000, () => {
  console.log(
    "🚀 Backend server (Groq AI Mode) is running on http://localhost:5000",
  );
});
